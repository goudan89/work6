// video11.cpp : 此文件包含 "main" 函数。程序执行将在此处开始并结束。
//
...
int sfp_control_thread(float& volumn, bool& volumnChange)
{
	...
	while (!thread_exit)
	{
		if (state[SDL_SCANCODE_A] && !key_plus_down) {
			key_plus_down = true;
			if (volumn < 50) {
				volumn += 0.1;
				cout << "音量升高" << endl;
		...
		else if (!state[SDL_SCANCODE_A] && key_plus_down) {
			key_plus_down = false;
		}

		if (state[SDL_SCANCODE_B] && !key_minus_down) {
			key_minus_down = true;
			if (volumn > 0) {
				volumn -= 0.1;
				cout << "音量降低" << endl;
		...
		else if (!state[SDL_SCANCODE_B] && key_minus_down) {
...
int sdlplayer(string filePath) {
	...
	//	SDL_WaitEvent(&event);
			...
			while (1) {
				...
				else if (event.type == SDL_KEYDOWN) //对视频的控制(前进
				{
